...
export default function UserListingsEnquiries() {
  ...
  useEffect(() => { get_user_create_listing(); }, []);
  ...
  const get_user_create_listing = () => {
    fetch(apiUrl + 'admin/get-user-create-listing', {
      ...
      .then((json) => { if (json.status) { setUserListingEnquiries(json.data); setShowNoData(false); } else { setShowNoData(true); } })
      ...
      .then((result) => {
        if (result.isConfirmed) {
          fetch(`${apiUrl}admin/update-user-create-listing/${id}`, {
            ...
            .then((res) => res.json())
            .then((json) => { if (json.status) { toast.success('Enquiry status updated successfully!'); setUserListingEnquiries((prev) => prev.map((e) => e.id === id ? { ...e, status: json.data.status } : e)); } else { toast.error('Failed to update.'); } })
  ...
  if (showNoData) return <NoDataFound message="No user listing enquiries found." showButton={false} />;

  return (
    <PageLayout title="User Listing Enquiries" subtitle="Manage listing requests submitted by users." flush>
            ...
            <div className="p-5">
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <h4 className="text-lg font-bold text-ink m-0 leading-tight">{enquiry.property_type || 'N/A'} Listing Request</h4>
                </div>
