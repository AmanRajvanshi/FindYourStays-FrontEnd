...
export default function Enquiries() {
  ...
  useEffect(() => { get_property_enquiries(); }, []);
  ...
  const get_property_enquiries = () => {
    fetch(apiUrl + 'admin/get-property-enquiries', {
      ...
      .then((json) => { if (json.status) { setPropertyEnquiries(json.data); setShowNoData(false); } else { setShowNoData(true); } })
      ...
      .then((result) => {
        if (result.isConfirmed) {
          fetch(`${apiUrl}admin/update-property-enquiries/${id}`, {
            ...
            .then((res) => res.json())
            .then((json) => { if (json.status) { toast.success('Enquiry status updated!'); setPropertyEnquiries((prev) => prev.map((e) => e.id === id ? { ...e, status: json.data.status } : e)); } else { toast.error('Failed to update.'); } })
  ...
  if (showNoData) return <NoDataFound message="No property enquiries found." showButton={false} />;

  return (
    <PageLayout title="Property Enquiries" subtitle="View and manage property enquiries from users." flush>
            ...
            <div className="p-5">
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <a href={`${domainUrl}single-property/${enquiry.property?.slug}`} target="_blank" rel="noopener noreferrer"
                    className="text-ink hover:text-coral transition-colors">
                    <h4 className="text-lg font-bold m-0 leading-tight">{enquiry.property?.property_title || 'N/A'}</h4>
                  </a>
                </div>
