...
export default function Cities() {
  ...
  const StatusCell = ({ rowData }) => {
    ...
    return (
      <span
        className={`badge ${status === 'active' ? 'bg-success' : 'bg-warning'}`}
      >
  ...
  const MainCityCell = ({ rowData }) => {
    ...
    return (
      <span className={`badge ${isMain ? 'bg-primary' : 'bg-secondary'}`}>
        {isMain ? 'Yes' : 'No'}
      </span>
    );
  };
      ...
      {showNoData && !searchTerm ? (
        ...
        <>
          <div className="d-flex justify-content-between align-items-center mb-4">
            ...
            <button className="btn btn-thm" onClick={() => handleOpen('city')}>
              + Add New City
            </button>
          </div>
